...
class Timestamp {
   public:
    ...
    std::string ToFormattedDefaultLogString() const {
        char buf[64] = {0};
        time_t seconds =
            static_cast<time_t>(micro_seconds_ / kMicrosecond2Second);
        struct tm tm_time;
    ...
    std::string ToFormattedString() const {
        ...
        localtime_r(&seconds, &tm_time);
        int micro_seconds =
            static_cast<int>(micro_seconds_ % kMicrosecond2Second);
...
inline Timestamp Timestamp::Now() {
    struct timeval time;
    // gettimeofday 时间精度达到1微妙 pow(10, -6) 满足基本时间需求 且是在用户态实现的 调用代价 相较于其他更高的代价的要小得多
    gettimeofday(&time, NULL);
    return Timestamp(time.tv_sec * kMicrosecond2Second + time.tv_usec);
}

inline Timestamp Timestamp::AddTime(const Timestamp& timestamp,
                                    double add_seconds) {
    int64_t add_microseconds =
        static_cast<int64_t>(add_seconds) * kMicrosecond2Second;
